...
```json
{
  ...
  "hatchedAt": "2024-01-08T10:00:30.000Z",
  "output": {
    "success": true,
...
| `isHatched` | `true` when execution is complete (success or failure) |
| `isSuccess` | `true` if completed successfully |
| `isFailed` | `true` if execution failed |
| `hatchedAt` | Timestamp when execution completed |
| `output` | Result payload (only when `isHatched: true`) |

<Note>
  The "incubate/hatch" metaphor: prompts are **incubated** (queued), then **hatch** when complete.
</Note>
...
| `COMPLETED` | Finished successfully (`isHatched: true`, `isSuccess: true`) |
| `FAILED` | Execution failed (`isHatched: true`, `isFailed: true`) |
